...
export const CreateNewManga = (): JSX.Element => {
  ...
  const { data, act, errMsg, status } = useAsync<string>(async () => await createNewMangaRequest(formData), false)
  ...
  return (
    <ContentBlock title='Criar novo manga' size={{ height: 'auto' }} >
      <Form onSubmit={handleSubmit} msg={data ?? errMsg}>
        ...
        <ButtonForm message='Criar Manga' />
      </Form>
    </ContentBlock>
  )
}
